...
function ApplicationHome() {
  ...
  function choose(application: "IBIM" | "YARDLOGIC") {
    ...
    if (!hasUnassignedWorkspace) clearAppSession();
  ...
  async function classify(id: string, applicationId: "IBIM" | "YARDLOGIC") {
    try {
      await api(`/auth/application/businesses/${id}`, { method: "PATCH", body: JSON.stringify({ applicationId }) });
      const updatedBusinesses = readStoredBusinesses().map((entry) => {
        const business = entry?.business ?? entry;
        if (business?.id !== id) return entry;
        if (entry?.business) return { ...entry, business: { ...entry.business, applicationId } };
