-- seed_ibim_demo_flexible.sql
-- 
-- iBIM demo data seeder for any IBIM business
-- Usage: psql -h your-host -U username -d database -f seed_ibim_demo_flexible.sql -v business_id='YOUR_BUSINESS_ID'
-- 
-- If using psql directly: ALTER the business_id variable below before running
-- Safe to rerun: demo rows are upserted by deterministic IDs/numbers.
--
-- To find your iBIM business ID, run:
--   SELECT id, name, "applicationId" FROM "Business" WHERE "applicationId" = 'IBIM';

BEGIN;

DO $$
DECLARE
  -- Set this to your actual iBIM business UUID
  -- Find it with: SELECT id FROM "Business" WHERE "applicationId" = 'IBIM';
  demo_business_id text := COALESCE(:'business_id', '2197c8f8-f0bf-48e4-bf65-8d01278a341d');
  row_number integer;
  member_id text;
  proposal_id text;
  policy_id text;
...
BEGIN
  -- Verify business exists and is IBIM
  IF NOT EXISTS (
    SELECT 1 FROM "Business"
    WHERE id = demo_business_id AND "applicationId" = 'IBIM'
  ) THEN
    RAISE EXCEPTION 'iBIM Business % not found or not classified as IBIM. Verify the business ID.', demo_business_id;
  END IF;

  -- Get the primary owner/user of this business
  ...
  -- Seed 100 demo members and policies
  ...
  -- Seed workflow proposals, tasks, and events (10 samples)
...
-- Verification: should return 100 members, 100 policies, 10 proposals, etc.
-- Replace 'YOUR_BUSINESS_ID' with your actual iBIM business UUID
SELECT
  (SELECT count(*) FROM "IbimMember" WHERE "businessId" = COALESCE(:'business_id', '2197c8f8-f0bf-48e4-bf65-8d01278a341d') AND "externalRef" LIKE 'DEMO-%') AS demo_members,
  (SELECT count(*) FROM "IbimPolicy" WHERE "businessId" = COALESCE(:'business_id', '2197c8f8-f0bf-48e4-bf65-8d01278a341d') AND "policyNumber" LIKE 'DEMO-POL-%') AS demo_policies,
  (SELECT count(*) FROM "IbimProposal" WHERE "businessId" = COALESCE(:'business_id', '2197c8f8-f0bf-48e4-bf65-8d01278a341d') AND id IN (SELECT md5('ibim-demo-proposal-' || n::text) FROM generate_series(1, 10) n)) AS demo_proposals,
  (SELECT count(*) FROM "IbimWorkflowTask" WHERE "businessId" = COALESCE(:'business_id', '2197c8f8-f0bf-48e4-bf65-8d01278a341d') AND note LIKE 'Demo action:%') AS demo_tasks,
  (SELECT count(*) FROM "IbimTransaction" WHERE "businessId" = COALESCE(:'business_id', '2197c8f8-f0bf-48e4-bf65-8d01278a341d') AND reference LIKE 'DEMO-TXN-%') AS demo_transactions,
  (SELECT count(*) FROM "AuditLog" WHERE "businessId" = COALESCE(:'business_id', '2197c8f8-f0bf-48e4-bf65-8d01278a341d') AND detail->>'source' = 'Neon demo seed') AS demo_audit;
